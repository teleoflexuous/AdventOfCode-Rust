    ...
    for (line_index, line) in char_matrix.iter().enumerate() {
        for (char_index, char) in line.iter().enumerate() {
            if char != &'*' {
                ...
                'point_check: for i in 0..3 {
                    if line_index + i < 1 || line_index + i > char_matrix.len() {
                        continue;
                    }
                    let square_line_index: usize = line_index + i - 1 as usize;
                    let line: Vec<char> = char_matrix[square_line_index].clone();
                    for j in 0..3 {
                        if char_index + j < 1 || char_index + j > line.len() {
                            continue;
                        }
                        if checked_squares.contains(&(square_line_index, char_index + j - 1)) {
                            continue;
                        } else {
                            checked_squares.push((square_line_index, char_index + j - 1));
                        }
                        ...
                        if char.is_numeric() {
                            ...
                            while right_char_index < line.len() - 1
                                && line[right_char_index + 1].is_numeric()
                            {
                                right_char_index += 1;
                            ...
                            if number_count == 0 {
                                gear_ratio = number.parse::<i32>().unwrap();
                                number_count += 1;
